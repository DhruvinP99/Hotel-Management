...
class Hotel {
    ...
    /**
     * Deallocates a room in the hotel.
     * 
     * @param rn    the room number to deallocate
     * @param rtype the type of room to deallocate (1 for luxury double room, 2 for
     *              deluxe double room, 3 for luxury single room, 4 for deluxe
     *              single room)
     */
    ...
    static void displayBookedRooms() {
        ...
        for (int i = 0; i < hotel_ob.luxury_doublerrom.length; i++) {
            if (hotel_ob.luxury_doublerrom[i] != null) {
                System.out.println("Room " + (i + 1) + ": " + hotel_ob.luxury_doublerrom[i].name);
            }
        }
        ...
        for (int i = 0; i < hotel_ob.deluxe_doublerrom.length; i++) {
            if (hotel_ob.deluxe_doublerrom[i] != null) {
                System.out.println("Room " + (i + 11) + ": " + hotel_ob.deluxe_doublerrom[i].name);
            }
        }
        ...
        for (int i = 0; i < hotel_ob.luxury_singleerrom.length; i++) {
            if (hotel_ob.luxury_singleerrom[i] != null) {
                System.out.println("Room " + (i + 31) + ": " + hotel_ob.luxury_singleerrom[i].name);
            }
        }
        ...
        for (int i = 0; i < hotel_ob.deluxe_singleerrom.length; i++) {
            if (hotel_ob.deluxe_singleerrom[i] != null) {
                System.out.println("Room " + (i + 41) + ": " + hotel_ob.deluxe_singleerrom[i].name);
            }
        }
    }
...
class write implements Runnable {
    ...
    /**
     * This method writes the hotel_ob object to a file named "backup" using
     * serialization.
     */
    @Override
    public void run() {
...
public class javaclassproject {
    public static void main(String[] args) {
        char continueChoice = 'y'; // Initialize continueChoice
        try {
            File backupFile = new File("backup");
            if (backupFile.exists()) {
                FileInputStream fileInputStream = new FileInputStream(backupFile);
                try (ObjectInputStream objectInputStream = new ObjectInputStream(fileInputStream)) {
                    Hotel.hotel_ob = (holder) objectInputStream.readObject();
                }
            }
                ...
                do {
                    ...
                    switch (choice) {
                        case 1:
                            ...
                            Hotel.features(roomType);
                            break;
                        case 2:
                            System.out.println(
                                    "Choose room type:\n1. Luxury Double Room\n2. Deluxe Double Room\n3. Luxury Single Room\n4. Deluxe Single Room\n");
                            roomType = scanner.nextInt();
                            Hotel.availability(roomType);
                            break;
                        case 3:
                            System.out.println(
                                    "Choose room type:\n1. Luxury Double Room\n2. Deluxe Double Room\n3. Luxury Single Room\n4. Deluxe Single Room\n");
                        ...
                        case 4:
                            ...
                            if (roomType > 60) {
                                ...
                                Hotel.order(roomType - 41, 4);
                            } else if (roomType > 30) {
                                Hotel.order(roomType - 31, 3);
                            } else if (roomType > 10) {
                                Hotel.order(roomType - 11, 2);
                            } else if (roomType > 0) {
                                Hotel.order(roomType - 1, 1);
                            } else {
                                System.out.println("Room doesn't exist");
                            }
                            break;
                        case 5:
                            ...
                            if (roomType > 60) {
                                ...
                                Hotel.deallocate(roomType - 41, 4);
                            } else if (roomType > 30) {
                                Hotel.deallocate(roomType - 31, 3);
                            } else if (roomType > 10) {
                                Hotel.deallocate(roomType - 11, 2);
                            } else if (roomType > 0) {
                                Hotel.deallocate(roomType - 1, 1);
                            } else {
                                System.out.println("Room doesn't exist");
                            }
                            break;
                    ...
                    while (!(continueChoice == 'y' || continueChoice == 'Y' || continueChoice == 'n'
                            || continueChoice == 'N')) {
                        System.out.println("Invalid option");
                        System.out.println("\nContinue: (y/n)");
                        continueChoice = scanner.next().charAt(0);
            ...
            Thread thread = new Thread(new write(Hotel.hotel_ob));
            thread.start();
        } catch (Exception e) {
